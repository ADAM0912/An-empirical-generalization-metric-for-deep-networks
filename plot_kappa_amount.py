...
def main():
    ...
    for j in range (len(zeroshot)):
        for i in range (len(nunits)):
            ...
            data_model_2_kappa = pd.read_excel(os.path.join('experiment result',
                                         str(nunits[i]) + str('_') + str(args.prune) + str('_') + str(args.test_epoch) + str('_zs') + str(
                                             zeroshot[j]) + str('_') + 'kappa_lp.xlsx'),header=None)
            #
            kappa_array = data_model_2_kappa[1][1:]
        ...
        labels = [ '1M', '2M', '4M', '8M', '16M', '32M', '64M']
    ...
    plt.title('Kappa with number of parameters changed',fontsize=16)
